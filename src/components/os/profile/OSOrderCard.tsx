...
export default function OSOrderCard({ os }: OSProfileProps) {
  const orderStatus = os.order_status ?? "EM ABERTO";
      ...
      <div className="mb-6 flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-sm text-gray-500 dark:text-gray-400">
            Dados comerciais relacionados à OS.
          </p>
        </div>
      </div>
        ...
        <DetailItem
          ...
          value={
            <div className="space-y-1">
              <p>{os.condicao_pagamento ?? "-"}</p>

              <p className="text-xs font-normal text-gray-500 dark:text-gray-400">
                {os.forma_pagamento ?? "-"} • {os.metodo_pagamento ?? "-"}
              </p>
            </div>
          }
        />
          ...
          value={
            <span
              className={`
                ...
                text-xs
                font-medium
                ${getStatusClassBadge(orderStatus)}
              `}
            >
              {orderStatus}
            </span>
        ...
        <DetailItem
          ...
          value={
            <span className="text-engeligas-500">
              {formatCurrency(os.total_pedido)}
            </span>
          }
        />
      </div>
